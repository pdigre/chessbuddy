...
describe('Move diff', () => {
  ...
  test('Promo white to queeen', () => {
    ...
    const fen2 = 'Q7/1K6/8/8/8/8/pk6/8';
    const move = FEN.detectMove(FEN.fen2brd(fen1), FEN.fen2brd(fen2));
    expect(move2string(move)).toBe('a7 a8');
  });

  test('Promo black to knight', () => {
    // Simple position: black bishop on c8, target square g4 empty.
    const fen1 = '8/PK6/8/8/8/8/pk6/8';
    const fen2 = '8/PK6/8/8/8/8/1k6/n7';
    const move = FEN.detectMove(FEN.fen2brd(fen1), FEN.fen2brd(fen2));
    expect(move2string(move)).toBe('a2 a1');
  });

  test('Castling white queeen', () => {
    // Simple position: black bishop on c8, target square g4 empty.
    const fen1 = 'r3k2r/8/8/8/8/8/8/R3K2R';
    const fen2 = 'r3k2r/8/8/8/8/8/8/2KR3R';
    const move = FEN.detectMove(FEN.fen2brd(fen1), FEN.fen2brd(fen2));
    expect(move2string(move)).toBe('e1 c1');
  });

  test('Castling white king', () => {
    // Simple position: black bishop on c8, target square g4 empty.
    const fen1 = 'r3k2r/8/8/8/8/8/8/R3K2R';
    const fen2 = 'r3k2r/8/8/8/8/8/8/R4RK1';
    const move = FEN.detectMove(FEN.fen2brd(fen1), FEN.fen2brd(fen2));
    expect(move2string(move)).toBe('e1 g1');
  });

  test('Castling black queeen', () => {
    // Simple position: black bishop on c8, target square g4 empty.
    const fen1 = 'r3k2r/8/8/8/8/8/8/R3K2R';
    const fen2 = '2kr3r/8/8/8/8/8/8/R3K2R';
    const move = FEN.detectMove(FEN.fen2brd(fen1), FEN.fen2brd(fen2));
    expect(move2string(move)).toBe('e8 c8');
  });

  test('Castling black king', () => {
    // Simple position: black bishop on c8, target square g4 empty.
    const fen1 = 'r3k2r/8/8/8/8/8/8/R3K2R';
    const fen2 = 'r4rk1/8/8/8/8/8/8/R3K2R';
    const move = FEN.detectMove(FEN.fen2brd(fen1), FEN.fen2brd(fen2));
