...
export const HistoryView = observer(({ gameHistory }: { gameHistory: HistoryService }) => {
  const historyClick = (event: MouseEvent<HTMLTableElement>) => {
    ...
    const id2 = id == gameHistory.markHist ? -1 : id;
    gameHistory.markHist = id2;
  };
  ...
  const tableRows = gameHistory.history
    ...
    .map((row, iRow) => {
      ...
      // {style + marker}
      const marker = iRow == gameHistory.markHist ? ' bg-green-300' : '';
      return (
        <tr
          key={iRow.toString()}
          id={iRow.toString()}
  ...
  return (
    <GridWidget onClick={historyClick} scroll={gameHistory.markHist == -1}>
      {tableRows}
    </GridWidget>
  );
});
